...
export default function BrandsPage() {
  const { data, error, isLoading, mutate } = useFetch<ApiResponse<Brand[]>>("/admin/brands")
  const [brands, setBrands] = useState<Brand[]>([])
  const [searchTerm, setSearchTerm] = useState("")
  const [isModalOpen, setIsModalOpen] = useState(false)
  const [selectedBrand, setSelectedBrand] = useState<Brand | undefined>()
  ...
  const handleSaveBrand = async (brandData: CreateBrandData) => {
    try {
      console.log("Saving brand:", { mode: selectedBrand ? "edit" : "add", brandData, selectedBrand }) // Debug log
