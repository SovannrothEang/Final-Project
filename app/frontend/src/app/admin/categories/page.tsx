...
import { createCategory, updateCategory, deleteCategory } from "@/app/api/category/category"
import useFetch from "@/utils/data-fetching"
import type { ApiResponse } from "@/types/api"
import Image from "next/image"

export default function CategoriesPage() {
  const { data, error, isLoading, mutate } = useFetch<ApiResponse<Category[]>>("/admin/categories")
  const [categories, setCategories] = useState<Category[]>([])
  const [searchTerm, setSearchTerm] = useState("")
  const [isModalOpen, setIsModalOpen] = useState(false)
  ...
  const handleSaveCategory = async (categoryData: CreateCategoryData) => {
    try {
      console.log("Saving category:", { mode: modalMode, categoryData, selectedCategory }) // Debug log
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 md:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{categories.reduce((sum, c) => sum + (c.products_count || 0), 0)}</div>
            <p className="text-xs text-muted-foreground">Across all categories</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {Math.round(categories.reduce((sum, c) => sum + (c.products_count || 0), 0) / (categories.length || 1))}
            </div>
            <p className="text-xs text-muted-foreground">Per category</p>
          </CardContent>
        </Card>
                ...
                {filteredCategories && filteredCategories.length > 0 ? (
                  filteredCategories.map((category) => (
                    <TableRow key={category.id}>
                      ...
                      <TableCell className="max-w-xs truncate">{category.description}</TableCell>
                      <TableCell>
                        <Badge variant="outline">{category.products_count || 0} products</Badge>
                      </TableCell>
                      <TableCell>
