    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 md:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{categories.reduce((sum, c) => sum + (c as any).productCount || 0, 0)}</div>
            <p className="text-xs text-muted-foreground">Across all categories</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {Math.round(categories.reduce((sum, c) => sum + (c as any).productCount || 0, 0) / (categories.length || 1))}
            </div>
            <p className="text-xs text-muted-foreground">Per category</p>
          </CardContent>
        </Card>
                ...
                <TableBody>
                  {filteredCategories.map((category) => (
                    <TableRow key={category.id}>
                      ...
                      <TableCell className="max-w-xs truncate">{category.description}</TableCell>
                      {/* <TableCell>
                        <Badge variant="outline">{(category as any).productCount || 0} products</Badge>
                      </TableCell> */}
                      <TableCell>
                        <Badge variant={category.is_active === true ? "default" : "secondary"}>{category.is_active==true ? "Active" : "In-active" }</Badge>
                      </TableCell>
