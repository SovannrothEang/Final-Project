import type { Brand, CreateBrandData, UpdateBrandData, BrandResponse } from "@/types/brands"
...
function convertBrandDataForAPI(data: CreateBrandData) {
  return {
    ...
    // Don't send logo field - you handle it separately
  }
}
...
export async function createBrand(payload: CreateBrandData): Promise<Brand> {
  ...
  console.log("Creating brand with payload:", apiPayload) // Debug log
  ...
  if (!res.ok) {
    ...
    try {
      ...
      throw new Error(`Failed to create brand: ${res.status} ${res.statusText} - ${errorText}`)
    }
  }
...
export async function updateBrand(id: number, payload: UpdateBrandData): Promise<Brand> {
  const headers = await getAuthHeaders()
  const apiPayload = convertBrandDataForAPI(payload)

  console.log("Updating brand:", { id, payload: apiPayload }) // Debug log
  ...
  if (!res.ok) {
    ...
    try {
      ...
      throw new Error(`Failed to update brand: ${res.status} ${res.statusText} - ${errorText}`)
    }
  }
