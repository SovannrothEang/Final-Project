...
interface BrandModalProps {
  ...
  brand?: Brand
  onBrandCreated: () => void
  onSave: (data: CreateBrandData) => Promise<void>
}

export function BrandModal({ isOpen, onClose, brand, onBrandCreated, onSave }: BrandModalProps) {
    ...
    if (isOpen) {
      if (brand) {
        setFormData({
          ...
          country: brand.country,
          website_url: brand.website_url,
          is_active: brand.is_active === 1, // Convert number to boolean
        })
      } else {
        setFormData({
          name: "",
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle>{brand ? "Edit Brand" : "Add New Brand"}</DialogTitle>
          <DialogDescription>
            {brand ? "Update the brand information below." : "Create a new brand to organize your products."}
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              placeholder="Enter country code (e.g., US, CH, CN)"
            />
          ...
          <DialogFooter>
            ...
            <Button type="submit" className="bg-red-600 hover:bg-red-700" disabled={isLoading}>
              {isLoading ? "Saving..." : brand ? "Update Brand" : "Add Brand"}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
